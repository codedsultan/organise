...
class ForgotPasswordController extends Controller
{
    ...
    public function sendResetLink(Request $request){
        ...
        $action_link = route('organiser.reset.password.form',['token'=>$token,'email'=>$request->email]);
    ...
    public function showResetForm(Request $request, $token = null){
        return view('dashboard.organiser.reset')->with(['token'=>$token,'email'=>$request->email]);
    }
    ...
    public function resetPassword(Request $request){
        ...
        if(!$check_token){
            ...
            Organiser::where('email', $request->email)->update([
            ...
            return redirect()->route('organiser.login')->with('info', 'Your password has been changed! You can login with new password')->with('verifiedEmail', $request->email);
        }
    }
}
